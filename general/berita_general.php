...
if ($action === 'delete' && $id > 0) {
  ...
  echo "<script>alert('Berita berhasil dihapus!'); window.location.href='berita_general.php';</script>";
  exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($error === '') {
    if ($mode === 'add') {
      ...
      echo "<script>alert('Berita berhasil ditambahkan!'); window.location.href='berita_general.php';</script>";
      exit();
    } else {
      $stmt = $conn->prepare("SELECT gambar FROM berita WHERE id = ? LIMIT 1");
      ...
      echo "<script>alert('Berita berhasil diperbarui!'); window.location.href='berita_general.php';</script>";
      exit();
    }
  }
}
...
if ($action === 'edit' && $id > 0) {
  ...
  if (!$editData) {
    echo "<script>alert('Data tidak ditemukan!'); window.location.href='berita_general.php';</script>";
    exit();
  }
}
      ...
      <div class="card-body">
        ...
        <form method="POST" enctype="multipart/form-data">
          ...
          <div class="row-btn">
            ...
            <?php if ($editData): ?>
              <a class="btn btn-ghost" href="berita_general.php">Batal Edit</a>
            <?php else: ?>
            ...
            <?php if ($total > 0): ?>
              <?php $no=1; while($b = $res->fetch_assoc()): ?>
                <tr>
                  ...
                  <td>
                    <div class="actions">
                      <a class="btn-sm btn-edit" href="berita_general.php?action=edit&id=<?php echo (int)$b['id']; ?>">Edit</a>
                      <a class="btn-sm btn-del" href="berita_general.php?action=delete&id=<?php echo (int)$b['id']; ?>"
